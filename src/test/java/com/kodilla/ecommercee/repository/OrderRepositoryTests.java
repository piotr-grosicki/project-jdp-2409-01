...
public class OrderRepositoryTests {
    ...
    void shouldSaveOrder() {
        // Given
        Order order = new Order(0, 100.0, LocalDate.now(), "NEW"); // Temporarily set orderId to 0 or a placeholder value
    ...
    void shouldDeleteOrder() {
        // Given
        Order order = new Order(0, 200.0, LocalDate.now(), "PENDING"); // Temporarily set orderId to 0 or a placeholder value
        Order savedOrder = orderRepository.save(order);
        Integer savedOrderId = savedOrder.getOrderId(); // Get the actual ID assigned by the database
        // When
